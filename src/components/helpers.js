...
export const actualizarStock = async (idArticulo, cantidad) => {
    try {
      ...
      const response = await fetch(`https://vivosis-back-v2.vercel.app/api/articulo/${idArticulo}`, {
        method: 'PUT',
        headers: {
  ...
  export const guardarCambiosEnProducto = async (idPedido, idArticulo, cantidad) => {
    try {
      ...
      if (stockActualizado) {
        ...
        const response = await fetch(`https://vivosis-back-v2.vercel.app/api/pedido/${idPedido}`, {
          method: 'DELETE',
        });
