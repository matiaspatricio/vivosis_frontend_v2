...
function ActualizaMasivaPedidos() {
  ...
  useEffect(() => {
    ...
    const fetchPedidos = async () => {
      try {
        const response = await axios.get('https://vivosis.vercel.app:3001/api/pedido/getPedidosPendientes');
  ...
  const handleGuardar = () => {
    ...
    pedidosActualizados.forEach(pedido => {
      axios.put(`https://vivosis.vercel.app:3001/api/pedido/${pedido._id}`, pedido)
        .then(response => {
